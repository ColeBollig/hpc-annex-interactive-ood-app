...
| `lib/annex_pilot.rb` | Queries the live, running pilot's own HTCondor config directly (any `condor_config_val` option); `require`d by `info.html.erb` |
...
`info.html.erb` shows two pieces of info on the session card in "My Interactive Sessions," sourced two different ways:
...
- **Idle-shutdown warning** — the execute point's actual configured `STARTD_NOCLAIM_SHUTDOWN`, converted to minutes. This one is **not** read from the tarball. Unlike `VERSION`, its value there can be silently overridden before it's ever applied — by the user's own `~/.condor/annex_config` (a shell override sourced by `annex-setup.sh`) or `~/.condor/annex_pilot_config` (an HTCondor `config.d` file copied in by `annex-job-setup.sh`), both real, supported customization points (see `USER-STEPS.md`). Enumerating every possible override isn't reliable, so `info.html.erb` queries the live HTCondor config system directly instead, via `lib/annex_pilot.rb`'s `AnnexPilot.condor_config_val(staged_root, option)` — a generic lookup that takes any `condor_config_val` option name, not just this one. It locates the pilot's own downloaded install under `staged_root` (`pilot.<jobid>/condor-<version-arch-distro>`, present once `annex-job-setup.sh` has finished assembling the pilot's full `config.d`, including any `annex_pilot_config` override) and runs `CONDOR_CONFIG=<pilot>/condor_config <pilot>/bin/condor_config_val <option>` directly (falling back to sourcing the pilot's own `condor.sh` if the config filename assumption turns out wrong). This queries fresh on every card render rather than once at submission — no waiting, no polling.

**Caveats:**
- `AnnexPilot` assumes the OOD web server can *execute* the pilot's downloaded HTCondor binary directly, not just read it. That binary was built for the compute node's OS/arch (`uname -m`/`/etc/os-release`, detected in `annex-setup.sh`, itself running on the compute node — see `template/before.sh.erb`), which may differ from the OOD web server's own. If it does, the query simply fails and returns `nil` (same as a pilot that doesn't exist yet) — the warning is just omitted, nothing breaks.
- Both `submit.yml.erb` and `info.html.erb` rely on `staged_root` being available as a local variable in their binding. That's real and confirmed working (traced in `ood/apps/dashboard`'s `BatchConnect::App#submit_opts`, and demonstrated in [a community example](https://discourse.openondemand.org/t/customizing-interactive-app-cards-using-erb/2694)), but it's **not part of OOD's documented API for these two files** (it's only officially documented for `template/*.sh.erb`) — an OOD maintainer has described these card templates as otherwise "pretty locked down." A future OOD release could change this without notice.
- **`__dir__` does not reliably resolve inside `info.html.erb` specifically** — confirmed by a real production `LoadError` (`cannot load such file -- ./lib/annex_pilot.rb`; `__dir__` evaluated to `"."`). `info.html.erb` and `submit.yml.erb` are rendered through two *different* OOD code paths (`BatchConnect::Session#render_info_view` vs. `BatchConnect::App#render_erb_file`), and only the latter sets up `__dir__` correctly. Workaround: `submit.yml.erb` writes its own `__dir__` (confirmed reliable there) into `annex_info.json` as `app_dir`, and `info.html.erb` reads that back instead of computing its own.
- All of this is wrapped in `rescue StandardError, ScriptError` (not `StandardError` alone) so the session card just silently omits this section rather than breaking, if any of it fails — `LoadError` (raised by a failed `require`, exactly the bug above) is a `ScriptError`, **not** a `StandardError`, so `rescue StandardError` alone silently fails to catch it. Caught the hard way, via a live crash, before this was tightened up.
